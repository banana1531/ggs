...
<body>
	...
	<table border="1" width="800" class="center">
		<tbody>
			...
			<%-- 반복문을 이용하여 줄출력 예정 --%>
			<c:forEach items="${InfoSS}" var="dto">
				<tr>
					<td>${dto.teamName}</td>
					<td>${dto.tel}</td>
					<td>${dto.homepage}</td>
	...
	<table border="1" width="800" class="center">
		<tbody>
			...
			<tr>
				<c:forEach items="${ScheduleSS}" var="dto">
					<tr>
						<td>${dto.gdate}</td>
						<td>${dto.gtime}</td>
						<td>${dto.ateamname}</td>
		...
		<tbody>
			<tr class="center">
				<td>
					...
					<a href="../team/SS?nowPage=${PINFO.nowPage-1}">[prev]</a> 
					</c:if>
					
					<c:forEach var="pg"	 begin="${PINFO.startPage}" end="${PINFO.endPage}">
					<a href="../team/SS?nowPage=${pg}">[${pg}]</a> 
					</c:forEach>
					...
					<a href="../team/SS?nowPage=${PINFO.nowPage+1}">[next]</a>
					</c:if>
				</td>
			</tr>
		</tbody>
