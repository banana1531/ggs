...
<script>
	$(document).ready(function(){
		...
		$("#btnReply").click(function(){
			...
			var wno = "${InfoErrorBoardDetail.writeno}"
	...
	function listReply(){
		$.ajax({
			type : "get",
			url : "../reply/list?wno=${InfoErrorBoardDetail.writeno}",
	...
	function listReply2(){
		$.ajax({
			...
			url : "../reply/listJson2?wno=${InfoErrorBoardDetail.writeno}",
			success : function(result){
				console.log(result);
	...
	function button_event(){
		if (confirm("정말 삭제하시겠습니까?") == true){
		    alert("삭제되었습니다");
		    location.href = "./delete?writeno=${InfoErrorBoardDetail.writeno}&nowPage=${nowPage}";
		}else{
		    return;
		}
	}
...
<body>
	<div class="container">
		<table class="table">
			<thead align="center">
				<tr>
					<td><h4>No.${InfoErrorBoardDetail.writeno}</h4></td>
					<td><h4>작성자 : ${InfoErrorBoardDetail.id}</h4></td>
					<td><h4>작성일 : <fmt:formatDate value="${InfoErrorBoardDetail.writedate}"
							pattern="yyyy-MM-dd HH:mm:ss"/></h4></td>
					<td><h4>조회수 : ${InfoErrorBoardDetail.views} Hits</h4></td>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td colspan="4"><h1>${InfoErrorBoardDetail.title}</h1></td>
				</tr>
				<tr>
					<td colspan="4"><h2>${fn:replace(InfoErrorBoardDetail.content, replaceChar, "<br/>")}</h2></td>
				</tr>
			</tbody>
		</table>
	</div>
	<div class="container" align="right">
		<c:if test="${UID == InfoErrorBoardDetail.id}">
		<button type="button" class="btn btn-primary" onclick="location.href='./update?writeno=${InfoErrorBoardDetail.writeno}&nowPage=${nowPage}'">수정하기</button>
					...
					<div class="modal-content">
						...
						<form class="form-inline" action="/decInsert.gg">
							<div class="modal-body" align="left">
								글번호 : <input name="writeno" value="${InfoErrorBoardDetail.writeno}"readonly="readonly" style="border: none;"><br /> 
								게시판 :<input name="boardname" value="${InfoErrorBoardDetail.boardname}"readonly="readonly" style="border: none;"><br /> 
								...
								<input type="hidden" name="views" value="${InfoErrorBoardDetail.views}">
							</div>
