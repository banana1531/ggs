...
<script>
	...
	function listReply(){
		$.ajax({
			type : "get",
			url : "../reply/list?wno=${noticeDetail.writeno}",
	...
	function listReply2(){
		$.ajax({
			...
			url : "../reply/listJson3?wno=${noticeDetail.writeno}",
			success : function(result){
				console.log(result);
	...
	function button_event(){
		if (confirm("정말 삭제하시겠습니까?") == true){
		    alert("삭제되었습니다");
		    location.href = "./delete?writeno=${noticeDetail.writeno}&nowPage=${nowPage}";
		}else{
		    return;
		}
	}
...
<div class="container">
	<table class="table">
		<thead align="center">
			<tr>
				<td><h4>No.${noticeDetail.writeno}</h4></td>
				<td><h4>작성자 : ${noticeDetail.id}</h4></td>
				<td><h4>작성일 : <fmt:formatDate value="${noticeDetail.writedate}"
						pattern="yyyy-MM-dd HH:mm:ss"/></h4></td>
				<td><h4>조회수 : ${noticeDetail.views} Hits</h4></td>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td colspan="4"><h1>${noticeDetail.title}</h1></td>
			</tr>
			<tr>
				<td colspan="4"><h2>${fn:replace(noticeDetail.content, replaceChar, "<br/>")}</h2></td>
			</tr>
		</tbody>
	</table>
</div>
<div class="container" align="right">
	<c:if test="${UID == 'admin'}">
	<button type="button" class="btn btn-primary" onclick="location.href='./update?writeno=${noticeDetail.writeno}&nowPage=${nowPage}'">수정하기</button>
