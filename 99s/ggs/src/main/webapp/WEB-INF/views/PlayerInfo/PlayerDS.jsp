 ...
 <a href="./HH">한화</a><br/>
	...
	<table border="1" width="800" class="center">
		<tbody>
			...
			<c:forEach items="${getListDS}" var="dto">
				<tr>
					<td>${dto.pno}</td>
					<td>${dto.name}</td>
					<td>${dto.labelno}</td>
		...
		<tbody>
			<tr class="center">
				<td>
					...
					<a href="../player/DS?nowPage=${PINFO.nowPage-1}">[prev]</a> 
					</c:if>
					
					<c:forEach var="pg"	 begin="${PINFO.startPage}" end="${PINFO.endPage}">
					<a href="../player/DS?nowPage=${pg}">[${pg}]</a> 
					</c:forEach>
					...
					<a href="../player/DS?nowPage=${PINFO.nowPage+1}">[next]</a>
					</c:if>
				</td>
			</tr>
		</tbody>
